...
exports.selectReplies = async(filter, cloud) => {
    ...
    var list = cloud.database().collection('articleReplies').where({
...
exports.delReply = async(replyId, cloud) => {
    ...
    let res = await cloud.database().collection('articleReplies').doc(replyId).get();
    console.log(res);
    if (res.data.openId == openId) {
        await cloud.database().collection("articleReplies").doc(replyId).remove();
        return true;
    } else {
        return false;
    }
...
exports.addReply = async(reply, cloud) => {
    ...
    await cloud.database().collection('articles').doc(reply.articleId).update({
        data: {
            reply: cloud.database().command.inc(1)
        }
    })
    return reply;
}
...
exports.delArticle = async(articleId, cloud) => {
    ...
    let openId = cloud.getWXContext().OPENID;
    let res = await cloud.database().collection('articles').doc(articleId).get();
    console.log(res);
    if (res.data.openId == openId) {
        await cloud.database().collection("articles").doc(articleId).remove();
        return true;
    } else {
        return false;
    }
