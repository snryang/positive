<template lang="pug">
    div
        van-sticky
            van-nav-bar(left-arrow @click-left="onClickLeft" title="管理会员资料")
                div(slot="right")
                    van-button(type="primary" size="small" @click="save") 保存
        div(class="fx-row")
            div(span="fx-1"): input(v-model="filter.userid" type="number" placeholder="会员ID")
            div(span="fx-1"): van-button(type="primary" size="mini" @click="loadUserDetail") 查询
            //- div(span="fx-1"): van-button(type="info" size="mini" @click="save") 保存
...
export default {
  ...
  methods: {
    ...
    async loadUserDetail() {
      ...
      let res = await api.v3.userdetail(_.trim(this.filter.userid))
      let res2 = await api.v3.lifephoto(_.trim(this.filter.userid))
      if (res.success) {
        this.lifePhotos = _.map(p => "https://bxc-1300253269.cos.ap-chengdu.myqcloud.com/" + p)(_.pluck("url")(res2.data || []))
    ...
    async save() {
      ...
      let res = await api.v3.saveUserdetail(entity);
      this.$toast.clear();
      if (res.success) {
